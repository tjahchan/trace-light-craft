...
export function NoteScreenshots({ screenshots, entryId, userId, onUploaded, onDeleted }: NoteScreenshotsProps) {
  ...
  const handleUpload = async (files: FileList | null) => {
    ...
    for (const file of Array.from(files)) {
      ...
      const path = `${userId}/notes/${entryId}/${crypto.randomUUID()}.${ext}`;
  ...
  }, [entryId, userId, screenshots.length]);
    ...
    <div className="space-y-2">
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 flex-wrap">
        ...
        <button
          ...
          disabled={uploading}
          className={cn(
            "shrink-0 w-32 h-20 rounded-lg border-2 border-dashed flex flex-col items-center justify-center gap-1 text-muted-foreground hover:text-foreground transition-colors",
            dragOver ? "border-primary/50 bg-primary/5" : "border-white/[0.1] hover:border-white/[0.2]",
